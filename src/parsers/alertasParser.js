...
export const ALERTAS_SHEET_COLUMNS = [
  'anio',
  'mes',
  'totalAlertas',
  'totalPositivos',
...
export function parseAlertasSheet(matrix) {
  ...
  return matrix
    .reduce((rows, row) => {
      ...
      // Future months can exist in the workbook before data is loaded.
      ...
      rows.push({
        ...
        mes: monthName,
        totalAlertas,
        totalPositivos: parseNumber(row[3]),
        porcentajePositivos: parseNumber(row[4]),
        aprehendidos: parseNumber(row[5]),
        elementosSecuestrados: parseNumber(row[6]),
...
export function isAlertasSheet(sheetName) {
  const normalizedSheetName = normalizeText(sheetName);

  return normalizedSheetName.includes('ALERTAS') &&
    normalizedSheetName.includes('POSITIVOS');
}
